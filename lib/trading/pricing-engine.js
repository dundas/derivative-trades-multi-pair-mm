  ...
  calculateGrossOrderPrice({ side, midPrice, spread, entryPrice = null, orderType = 'limit', isTakeProfit = false }) {
    ...
    try {
      if (side === 'buy') {
        ...
        switch(buyMode) {
          ...
          case "MIDPOINT_OFFSET":
            if (typeof configPercentage !== 'number' || isNaN(configPercentage)) {
              ...
              // For buy orders, place BELOW midpoint to avoid crossing spread
              price = midPrice * (1 - configPercentage);
              
              // Ensure we don't cross the spread
              const bestAsk = midPrice + (spread / 2);
              if (price >= bestAsk) {
                price = bestAsk - 0.00001; // Small buffer to ensure maker order
                this.logger.warn('[PricingEngine] MIDPOINT_OFFSET buy price adjusted to avoid crossing spread', {
                  originalPrice: midPrice * (1 - configPercentage),
                  adjustedPrice: price,
                  bestAsk,
                  spread
                });
            ...
            break;
          case "FEE_ADJUSTED":
            // For FEE_ADJUSTED, use maker fee rate to ensure profitable maker orders
            const makerFeeRate = this.actualExchangeFeeRates.maker !== null ? this.actualExchangeFeeRates.maker : 0;
            // Place buy order below midPrice by maker fee amount to ensure profitability
            price = midPrice * (1 - makerFeeRate);
            
            // Additional spread consideration to ensure maker execution
            const bestAskFeeAdj = midPrice + (spread / 2);
            if (price >= bestAskFeeAdj) {
              price = bestAskFeeAdj - 0.00001;
            }

            this.logger.debug('[PricingEngine] BUY FEE_ADJUSTED pricing', {
              actualMakerFeeRate: makerFeeRate,
              midPrice,
              calculatedPrice: price,
              bestAsk: bestAskFeeAdj
            });
            break;
          case "TAKER_PRICE": // For a buy market order, takes liquidity at the market price
            ...
            // Skip spread validation for TAKER_PRICE as it's intentionally designed to cross
            return price;
          default:
        ...
        // Final validation: ensure buy order doesn't cross spread
        const bestAsk = midPrice + (spread / 2);
        if (price >= bestAsk) {
          const originalPrice = price;
          price = bestAsk - 0.00001; // Small buffer to ensure maker order
          this.logger.warn('[PricingEngine] Buy order price adjusted to avoid crossing spread', {
            strategy: buyMode,
            originalPrice,
            adjustedPrice: price,
            bestAsk,
        ...
        switch(sellMode) {
          ...
          case "SPREAD_CAPTURE": // Sell at a price that captures the full spread from entry
            ...
            break;
          case "MARKET_EDGE": // Standard sell slightly above best ask to ensure maker
            // Place slightly above current best ask to avoid immediate taker execution
            const currentBestAsk = midPrice + (spread / 2);
            price = currentBestAsk + 0.00001; // Small buffer above best ask
            break;
          case "INSIDE_MARKET": // Sell between midpoint and best ask
            // Place at 75% of the way from midpoint to best ask (safer maker positioning)
            const bestAskIM = midPrice + (spread / 2);
            price = midPrice + (spread * 0.375); // 3/8 of spread above midpoint
            
            // Ensure we don't get too close to best ask
            if (price >= bestAskIM - 0.00001) {
              price = bestAskIM - 0.00001;
            }
            break;
        ...
        // Final validation: ensure sell order doesn't cross spread
        const bestBid = midPrice - (spread / 2);
        if (price <= bestBid) {
          const originalPrice = price;
          price = bestBid + 0.00001; // Small buffer to ensure maker order
          this.logger.warn('[PricingEngine] Sell order price adjusted to avoid crossing spread', {
            strategy: sellMode,
            originalPrice,
            adjustedPrice: price,
            bestBid,
