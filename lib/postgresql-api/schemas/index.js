...
export const SESSIONS_SCHEMA = {
  ...
  columns: {
    ...
    // Multi-pair support (added in migration 001)
    pairs: {
      type: 'JSONB',
      nullable: true,
      default: '[]',
      description: 'Array of trading pairs in this session (e.g., ["BTC/USD", "ETH/USD"])'
    },
    pair_count: {
      type: 'INTEGER',
      nullable: true,
      default: 0,
      description: 'Number of trading pairs in this session (denormalized from pairs array)'
    },
  ...
  indexes: [
    ...
    // Multi-pair indexes (added in migration 002)
    { name: 'idx_sessions_pair_count', columns: ['pair_count'] },
    { name: 'idx_sessions_pairs_gin', columns: ['pairs'], type: 'GIN' },
    { name: 'idx_sessions_settlement_discovery', columns: ['settlesession', 'settledcomplete', 'startedat'] }
  ]
};
...
export const ORDERS_SCHEMA = {
  ...
  columns: {
    ...
    pair_index: {
      ...
      nullable: true,
      default: 0,
      description: 'Index of the pair in the session pairs array (0-based)'
    },
  ...
  indexes: [
    ...
    // Multi-pair indexes (added in migration 002)
    { name: 'idx_orders_session_symbol', columns: ['sessionid', 'symbol'] },
...
export const FILLS_SCHEMA = {
  ...
  indexes: [
    ...
    // Multi-pair indexes (added in migration 002)
    { name: 'idx_fills_session_symbol', columns: ['sessionid', 'symbol'] },
    { name: 'idx_fills_session_order_symbol', columns: ['sessionid', 'orderid', 'symbol'] },
    { name: 'idx_fills_symbol_timestamp', columns: ['symbol', 'timestamp'] },
    { name: 'idx_fills_unique_dedup', columns: ['sessionid', 'orderid', 'symbol', 'timestamp', 'price', 'size'], unique: true }
  ]
};
